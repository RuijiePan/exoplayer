 ...
 **/
public class GoGpuImageRenderer implements GLSurfaceView.Renderer, GLTextureView.Renderer, Camera.PreviewCallback {
    ...
    private int surfaceTextureId = NO_IMAGE;
    ...
    public GoGpuImageRenderer(final GPUImageFilter filter) {
        this.filter = filter;
    ...
    private void initTexture() {
        ...
        surfaceTexture.setOnFrameAvailableListener(surfaceTexture -> {
            if (renderCallback != null)
                renderCallback.onFrameAvailable(surfaceTexture.getTimestamp());
            });
        if (renderCallback != null) {
            renderCallback.onSurfaceTextureCreated(surfaceTexture);
        }
    }
    ...
    public void onDrawFrame(final GL10 gl) {
        ...
        runAll(runOnDraw);
        filter.onDraw(surfaceTextureId, glCubeBuffer, glTextureBuffer);
        runAll(runOnDrawEnd);
    ...
    public void setFilter(final GPUImageFilter filter) {
        runOnDraw(new Runnable() {
            ...
            @Override
            public void run() {
                final GPUImageFilter oldFilter = GoGpuImageRenderer.this.filter;
                GoGpuImageRenderer.this.filter = filter;
                if (oldFilter != null) {
                    oldFilter.destroy();
                }
                GoGpuImageRenderer.this.filter.ifNeedInit();
                GLES20.glUseProgram(GoGpuImageRenderer.this.filter.getProgram());
                GoGpuImageRenderer.this.filter.onOutputSizeChanged(outputWidth, outputHeight);
            }
        });
    }
    ...
    private void adjustImageScaling() {
        ...
        Log.w("ruijie", "width = " + ratioWidth + ", height = " + ratioHeight);
        float[] cube = CUBE;
