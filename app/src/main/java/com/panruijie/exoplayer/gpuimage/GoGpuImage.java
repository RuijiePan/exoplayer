...
public class GoGpuImage {
    ...
    private final GoGpuImageRenderer renderer;
    private int surfaceType = SURFACE_TYPE_SURFACE_VIEW;
    private GLSurfaceView glSurfaceView;
    private GLTextureView glTextureView;
    private GPUImageFilter filter;
    private Bitmap currentBitmap;
    private GoGpuImage.ScaleType scaleType = GoGpuImage.ScaleType.CENTER_CROP;

    /**
     * Instantiates a new GoGpuImage object.
     *
     * @param context the context
     */
    public GoGpuImage(final Context context, IRenderCallback callback) {
        ...
        renderer = new GoGpuImageRenderer(filter);
        renderer.setRenderCallback(callback);
    }

    public void setInputInfo(int width, int height) {
        renderer.setInputInfo(width, height);
    }
    ...
    /**
     * This sets the scale type of GoGpuImage. This has to be run before setting the image.
     * If image is set and scale type changed, image needs to be reset.
     *
     * @param scaleType The new ScaleType
     */
    public void setScaleType(GoGpuImage.ScaleType scaleType) {
    ...
    public void setImage(final Uri uri) {
        new GoGpuImage.LoadImageUriTask(this, uri).execute();
    }
    ...
    public void setImage(final File file) {
        new GoGpuImage.LoadImageFileTask(this, file).execute();
    }
    ...
    public Bitmap getBitmapWithFilterApplied(final Bitmap bitmap, boolean recycle) {
        ...
        GoGpuImageRenderer renderer = new GoGpuImageRenderer(filter);
    ...
    public static void getBitmapForMultipleFilters(final Bitmap bitmap,
                                                   final List<GPUImageFilter> filters, final GoGpuImage.ResponseListener<Bitmap> listener) {
        if (filters.isEmpty()) {
            return;
        }
        GoGpuImageRenderer renderer = new GoGpuImageRenderer(filters.get(0));
        renderer.setImageBitmap(bitmap, false);
        PixelBuffer buffer = new PixelBuffer(bitmap.getWidth(), bitmap.getHeight());
        buffer.setRenderer(renderer);
    ...
    public void saveToPictures(final String folderName, final String fileName,
                               final GoGpuImage.OnPictureSavedListener listener) {
    ...
    public void saveToPictures(final Bitmap bitmap, final String folderName, final String fileName,
                               final GoGpuImage.OnPictureSavedListener listener) {
        new GoGpuImage.SaveTask(bitmap, folderName, fileName, listener).execute();
    }
    ...
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        private final GoGpuImage.OnPictureSavedListener listener;
    ...
    private class LoadImageUriTask extends GoGpuImage.LoadImageTask {

        private final Uri uri;

        public LoadImageUriTask(GoGpuImage gpuImage, Uri uri) {
            super(gpuImage);
            this.uri = uri;
        }
    ...
    private class LoadImageFileTask extends GoGpuImage.LoadImageTask {

        private final File imageFile;

        public LoadImageFileTask(GoGpuImage gpuImage, File file) {
            super(gpuImage);
            imageFile = file;
        }
    ...
    private abstract class LoadImageTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        private final GoGpuImage gpuImage;
        private int outputWidth;
        private int outputHeight;

        public LoadImageTask(final GoGpuImage gpuImage) {
        ...
        private Bitmap scaleBitmap(Bitmap bitmap) {
            ...
            if (scaleType == GoGpuImage.ScaleType.CENTER_CROP) {
                // Crop it
        ...
        private int[] getScaleSize(int width, int height) {
            ...
            boolean adjustWidth = scaleType == GoGpuImage.ScaleType.CENTER_CROP
